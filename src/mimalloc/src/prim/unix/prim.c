...
void *(*origin_mmap)(void *addr, size_t length, int prot, int flags,
                  int fd, off_t offset) = mmap;
int (*origin_munmap)(void *addr, size_t length) = munmap;
...
int _mi_prim_free(void* addr, size_t size ) {
  bool err = (origin_munmap(addr, size) == -1);
  return (err ? errno : 0);
}
...
static void* unix_mmap_prim(void* addr, size_t size, size_t try_alignment, int protect_flags, int flags, int fd) {
  ...
  if (addr == NULL && try_alignment > 1 && (try_alignment % _mi_os_page_size()) == 0) {
    ...
    if (((size_t)1 << n) == try_alignment && n >= 12 && n <= 30) {  // alignment is a power of 2 and 4096 <= alignment <= 1GiB
      p = origin_mmap(addr, size, protect_flags, flags | MAP_ALIGNED(n), fd, 0);
      ...
      prctl(PR_SET_VMA, PR_SET_VMA_ANON_NAME, p, size, "utrace");
  ...
  if (addr == NULL && try_alignment > 1 && (try_alignment % _mi_os_page_size()) == 0) {
    p = origin_mmap((void*)try_alignment, size, protect_flags, flags | MAP_ALIGN, fd, 0);  // addr parameter is the required alignment
    if (p!=MAP_FAILED) return p;
    // fall back to regular mmap
  }
  #endif
  ...
  if (addr == NULL) {
    ...
    if (hint != NULL) {
      p = origin_mmap(hint, size, protect_flags, flags, fd, 0);
      if (p==MAP_FAILED || !_mi_is_aligned(p,try_alignment)) { 
  ...
  #endif
  // regular mmap
  p = origin_mmap(addr, size, protect_flags, flags, fd, 0);
  if (p!=MAP_FAILED) return p;
  // failed to allocate
  return NULL;
}
