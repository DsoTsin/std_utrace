...
//		static void ClearEntries(EntryType* Entries, int EntryCount);	// Fill an array of entries with empty values
//
...
class TGrowOnlyLockFreeHash
{
	...
	using FScopeLock = std::scoped_lock<FCriticalSection>;
public:
	...
	void Reserve(int Count)
	{
		...
		FHashHeader* HashTableLocal = (FHashHeader*)malloc(sizeof(FHashHeader) + (Count - 1) * sizeof(EntryType));
	...
	void Emplace(KeyType Key, ValueType Value, bool* bIsAlreadyInTable = nullptr)
	{
		...
		{
			int GrowCount = HashTableLocal ? HashTableLocal->TableSize * 2 : DEFAULT_INITIAL_SIZE;
			FHashHeader* HashTableGrow = (FHashHeader*)malloc(sizeof(FHashHeader) + (GrowCount - 1) * sizeof(EntryType));
			...
			if (HashTableLocal)
			{
				...
				for (int TableIndex = 0; TableIndex < HashTableLocal->TableSize; TableIndex++)
				{
...
private:
	...
	static constexpr int DEFAULT_INITIAL_SIZE = 1024;

	static void HashInsertInternal(FHashHeader* HashTableLocal, KeyType Key, ValueType Value)
	{
		int TableMask = HashTableLocal->TableSize - 1;

		// Linear probing
		for (int TableIndex = EntryType::KeyHash(Key) & TableMask;; TableIndex = (TableIndex + 1) & TableMask)
		{
