...
#include "Runtime/TraceLog/Public/Trace/Detail/Atomic.h"
#include "Runtime/TraceLog/Public/Trace/Detail/Channel.h"
#include "Runtime/TraceLog/Public/Trace/Detail/Protocol.h"
#include "Runtime/TraceLog/Public/Trace/Detail/Transport.h"
...
static bool Writer_UpdateConnection()
{
	...
	if (GPendingDataHandle >= ((UPTRINT)~0ull - CloseInertia))
	{
		--GPendingDataHandle;
...
{
	ThreadRegister("Trace", 0, INT_MAX);
...
bool Writer_WriteSnapshotTo(const ANSICHAR* Path)
{
	...
	{
		TStashGlobal<UPTRINT> DataHandle(GDataHandle);
		TStashGlobal<UPTRINT> PendingDataHandle(GPendingDataHandle);
		TStashGlobal<int8> SyncPacketCountdown(GSyncPacketCountdown, GNumSyncPackets);
		TStashGlobal<FStatistics> TraceStatistics(GTraceStatistics);
