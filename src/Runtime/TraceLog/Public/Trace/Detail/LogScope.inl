...
inline void FLogScope::EnterPrelude(uint32 Size)
{
	...
	if (UT_UNLIKELY(Buffer->Cursor + AllocSize > (uint8*)Buffer))
	{
...
inline FScopedLogScope::~FScopedLogScope()
{
	...
	if (UT_UNLIKELY(int32((uint8*)Buffer - Buffer->Cursor) < int32(sizeof(LeaveUid))))
	{
		Buffer = Writer_NextBuffer();
	}
...
inline FScopedStampedLogScope::~FScopedStampedLogScope()
{
	...
	if (UT_UNLIKELY(int32((uint8*)Buffer - Buffer->Cursor) < int32(sizeof(Stamp))))
	{
		Buffer = Writer_NextBuffer();
	}
...
FORCENOINLINE auto FLogScope::ScopedEnter()
{
	...
	FWriteBuffer* Buffer = Writer_GetBuffer();
	if (UT_UNLIKELY(int32((uint8*)Buffer - Buffer->Cursor) < int32(sizeof(EnterUid))))
	{
		Buffer = Writer_NextBuffer();
	}
