...
#define TRACE_PRIVATE_EVENT_BEGIN_IMPL(LoggerName, EventName, ...) \
	...
	{ \
		...
		static constexpr uint32_t GetSize() { return EventProps_Meta::Size; } \
		static uint32_t GetUid() { static uint32_t Uid = 0; return (Uid = Uid ? Uid : Initialize()); } \
		static uint32_t FORCENOINLINE Initialize() \
		{ \
			static const uint32_t Uid_ThreadSafeInit = [] () \
			{ \
...
#define TRACE_PRIVATE_EVENT_FIELD(FieldType, FieldName) \
		...
		template <typename... Ts> auto FieldName(Ts... ts) const { \
			LogScopeType::FFieldSet<FieldName##_Meta, FieldType>::Impl((LogScopeType*)this, std::forward<Ts>(ts)...); \
			return true; \
		} \
...
#define TRACE_PRIVATE_EVENT_END() \
		...
		static constexpr uint16_t NumDefinitionFields = (bIsDefinition != 0) ? 1 : 0;\
		...
		static_assert( \
			(bIsImportant == 0) || (uint32_t(EventFlags) & uint32_t(UE::Trace::Private::FEventInfo::Flag_NoSync)), \
