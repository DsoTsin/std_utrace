...
UE_TRACE_API bool	SendTo(const char* Host, uint32 Port=0) UE_TRACE_IMPL(false);
UE_TRACE_API bool	WriteTo(const char* Path) UE_TRACE_IMPL(false);
UE_TRACE_API bool	IsTracing() UE_TRACE_IMPL(false);
UE_TRACE_API bool	Stop() UE_TRACE_IMPL(false);
UE_TRACE_API bool	IsChannel(const char* ChanneName) UE_TRACE_IMPL(false);
UE_TRACE_API bool	ToggleChannel(const char* ChannelName, bool bEnabled) UE_TRACE_IMPL(false);
UE_TRACE_API void	EnumerateChannels(ChannelIterFunc IterFunc, void* User);
UE_TRACE_API void	ThreadRegister(const char* Name, uint32 SystemId, int32 SortHint) UE_TRACE_IMPL();
UE_TRACE_API void	ThreadGroupBegin(const char* Name) UE_TRACE_IMPL();
